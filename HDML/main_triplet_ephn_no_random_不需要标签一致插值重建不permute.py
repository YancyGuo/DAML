...
def main(_):
    ...
    if FLAGS.Apply_HDML:
        ...
        concat_embeddings, concat_labels = HDML_n_pnt.Pulling(FLAGS.LossType, embedding_z, label)  # 4组样本  256维
        ...
        with tf.variable_scope('Generator'):
            ...
            embedding_yq = tf.gather(embedding_y_concate, tf.range(FLAGS.batch_size, FLAGS.batch_size + tf.shape(concat_embeddings)[0]))
        ...
        with tf.name_scope('Loss'):
            ...
            syn_sample_loss_loss = Loss_ops.Loss(embedding_z_quta, concat_labels, 0.1, _lossType='hard_neg_mining_epnh_synstage_loss')
            ...
            # J_recon = (1 - FLAGS._lambda) * tf.reduce_sum(tf.square(embedding_yp_orisample - embedding_y_origin_permute))  # 1024维
            J_recon = (1 - FLAGS._lambda) * tf.reduce_sum(tf.square(embedding_yp_orisample - embedding))  # 1024维
            ...
            for batch in copy.copy(epoch_iterator):
                ...
                if not FLAGS.Apply_HDML:
                    ...
                    pbar.set_description("J_metric_var: %f, Jm: %f, J_syn_var: %f, "
                                         "J_recon_var: %f, J_gen_var: %f, syn_factor: %f" %
                                         (J_metric_var, J_m_var, J_syn_var, J_recon_var, J_gen_var, _syn_factor))
