...
import os
from lib.Loss_ops import distMC
...
def Pulling(Loss_type, embedding, Jm):
    ...
    #     return embedding_z_quta, label
        ...
        allSample = sample_split[0]  # （？，256）
        samples = tf.split(allSample, 2, axis=0)  # （？，256）
        ...
        # embedding_split = tf.split(embedding, 3, axis=0)  # （？，256）
        # all_sample = embedding_split[0]  # （？，256）
        # pos = embedding_split[1]  # （？，256）
        # neg = embedding_split[2]  # （？，256）
        # class0_emb, class1_emb = tf.split(all_sample, 2, axis=0)
        #
        #
        class0_class1_dist = distMC(class0_emb, class1_emb)
        # class1_dist = distMC(class1_emb, class1_emb)
        #
        # # 三点中心
        # farthest_idx = tf.arg_max(class0_dist, 1)
        # # farthest_idx = tf.Print(farthest_idx, [farthest_idx], "farthest_idx", summarize=1000)
        #
        # least_idx = tf.arg_min(class0_dist, 1) # 相似度最小的，即距离最大的
        # # least_idx = tf.Print(least_idx, [least_idx], "least_idx", summarize=1000)
        # farthest = tf.gather(allSample, farthest_idx)
        # # farthest = tf.Print(farthest, [farthest], "farthest", summarize=1000)
        # least = tf.gather(allSample, least_idx)
        # # least = tf.Print(least, [least], "least", summarize=1000)
        # allSample = (allSample + farthest + least)/3
        # # gen_class0 = tf.Print(gen_class0, [gen_class0], "gen_class0", summarize=1000)
        ...
        # # embedding2 is the pulled embedding
        # # embedding_z_quta = tf.concat([gen_class0, class1_emb], axis=0)
        # embedding_z_quta = tf.concat([gen_class0,class1_emb, pos, neg], axis=0)

        # gen_sample = allSample + 0.05 * (allSample - neg)
        # gen_neg = neg + 0.05*(neg - allSample)
        # gen_neg = (pos + allSample)/2

        center_0 = tf.reduce_mean(class0_emb, axis=0)
        center_1 = tf.reduce_mean(class1_emb, axis=0)
        center = center_0 + center_1
        gen_class0 = (center + class0_emb) / 2


        # gen_class0 = gen_by_anc_maxSimilar(class0_emb, class0_class1_dist)
        # gen_class1 = gen_by_anc_minSimilar(class1_emb, class1_dist)
        # gen_class0 = capsule(class0_emb)
        embedding_z_quta = tf.concat([gen_class0, class1_emb, pos, neg], axis=0)
        return embedding_z_quta
    else:
        print("Your loss type is not suit for HDML")
