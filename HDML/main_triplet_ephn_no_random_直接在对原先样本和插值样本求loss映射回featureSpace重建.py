...
from lib import GoogleNet_Model, Loss_ops, nn_Ops, HDML_n_pnt, evaluation
import copy
...
def main(_):
    ...
    if FLAGS.Apply_HDML:
        ...
        concat_embeddings, concat_labels = HDML_n_pnt.Pulling(FLAGS.LossType, embedding_z, label)  # 4组样本  256维  原来的+插值点  用来度量
        ...
        with tf.name_scope('Loss'):
            ...
            J_metric = Loss_ops.Loss(concat_embeddings, concat_labels, 0.1, _lossType='hard_neg_mining_epnh_synstage_loss')
        ...
        with tqdm(total=FLAGS.max_steps) as pbar:
            for batch in copy.copy(epoch_iterator):
                ...
                if not FLAGS.Apply_HDML:
                    ...
                    c_train, g_train,  J_metric_var, \
                    J_recon_var, J_gen_var,  _label_raw,\
                         = sess.run(
                        [c_train_step, g_train_step,
                         J_metric, J_recon, J_gen,  label_raw
                        ],
                    ...
                    pbar.set_description("J_metric_var: %f"
                                         ...
                                         (J_metric_var,  J_recon_var, J_gen_var))
