...
from lib import GoogleNet_Model, Loss_ops_copy, nn_Ops_copy, Embedding_Visualization, HDML_copy, evaluation
import copy
...
streams = data_provider.get_streams(FLAGS.batch_size, FLAGS.dataSet, 'Triplet', crop_size=FLAGS.default_image_size)
...
def main(_):
    ...
    with tf.variable_scope('Classifier'):
        ...
        with tf.name_scope('Loss'):
            ...
            J_m = Loss_ops_copy.Loss(embedding_z, label, FLAGS.LossType) + wdLoss
    ...
    if FLAGS.Apply_HDML:
        ...
        embedding_z_quta, factor = HDML_copy.Pulling(FLAGS.LossType, embedding_z, Javg)
        print("embedding_z_quta shape:", embedding_z_quta.shape)
        embedding_z_concate = tf.concat([embedding_z, embedding_z_quta], axis=0)
        # print("embedding_z_concate shape:", embedding_z_concate.shape)

        # Generator
        with tf.variable_scope('Generator'):
            # generator fc3
            embedding_y_concate = nn_Ops_copy.fc_block(
        ...
        with tf.name_scope('Loss'):
            J_syn = (1. - tf.exp(-FLAGS.beta / Jgen)) * Loss_ops_copy.Loss(embedding_z_quta, label,
                                                                           _lossType=FLAGS.LossType)
            J_m = (tf.exp(-FLAGS.beta / Jgen)) * J_m
            J_metric = J_m + J_syn

            cross_entropy, W_fc, b_fc = HDML_copy.cross_entropy(embedding=embedding_y_origin, label=label)
    ...
    if FLAGS.Apply_HDML:
        c_train_step = nn_Ops_copy.training(loss=J_metric, lr=lr, var_scope='Classifier')
        g_train_step = nn_Ops_copy.training(loss=J_gen, lr=FLAGS.lr_gen, var_scope='Generator')
        s_train_step = nn_Ops_copy.training(loss=cross_entropy, lr=FLAGS.s_lr, var_scope='Softmax_classifier')
    else:
        train_step = nn_Ops_copy.training(loss=J_m, lr=lr)
    ...
    with tf.Session(config=config) as sess:
        ...
        # collectors
        J_m_loss = nn_Ops_copy.data_collector(tag='Jm', init=1e+6)
        J_syn_loss = nn_Ops_copy.data_collector(tag='J_syn', init=1e+6)
        J_metric_loss = nn_Ops_copy.data_collector(tag='J_metric', init=1e+6)
        J_soft_loss = nn_Ops_copy.data_collector(tag='J_soft', init=1e+6)
        J_recon_loss = nn_Ops_copy.data_collector(tag='J_recon', init=1e+6)
        J_gen_loss = nn_Ops_copy.data_collector(tag='J_gen', init=1e+6)
        cross_entropy_loss = nn_Ops_copy.data_collector(tag='cross_entropy', init=1e+6)
        wd_Loss = nn_Ops_copy.data_collector(tag='weight_decay', init=1e+6)
        max_nmi = 0
        step = 0
        max_f1_score = 0
        ...
        with tqdm(total=FLAGS.max_steps) as pbar:
            for batch in copy.copy(epoch_iterator):
                ...
                x_batch_data, Label_raw = nn_Ops_copy.batch_data(batch)
                pbar.update(1)
                if not FLAGS.Apply_HDML:
                    ...
                    # tf.summary.histogram('histogram', fddf)
                step += 1
                ...
                if step % bp_epoch == 0:
                    ...
                    # nmi_tr, f1_tr, recalls_tr = evaluation.Evaluation( stream_train_eval, image_mean, sess, x_raw,
                    # label_raw, is_Training, embedding_z, 98, neighbours)
