...
def new_npair_loss(labels, embedding_anchor, embedding_positive,
                   reg_lambda, equal_shape=True, half_batch_size=FLAGS.batch_size//2):
    """
...
def easy_positive_hard_negative_loss(embeddings, labels, temperate,
                                     reg_lambda=3e-3, print_losses=False):
    """
    ...
    # 把异类对的距离设为最大
    # D_Positive[Diff] = -1
    dis_mask = tf.ones_like(D_Positive)  # 把异类对的距离设为最大
    D_Positive = tf.where(Diff, 0-dis_mask, D_Positive)
    # D_Positive = tf.Print(D_Positive, [D_Positive, dis_mask, Diff], message='Debug message:', summarize=1000)
    # D_Positive = tf.multiply(tf.cast(same, tf.float32), D_Positive) - tf.cast(Diff, tf.float32)
...
def hard_neg_mining_epnh_synstage_loss(l2norm_emb, labels, temperate,
                                     reg_lambda=3e-3, print_losses=False):
    """
    所有样本作为anchor,然后在里面找positive和negative,除了对角线
    :param l2norm_emb:
    :param labels:
    :param temperate:
    :param reg_lambda:
    ...
    dist_ap = tf.gather(D_Positive, indx, axis=0)    # 前18行
    dist_ap = tf.gather(dist_ap, indx, axis=1)   # 前18列

    V_pos = tf.reduce_max(dist_ap, axis=1)
    I_pos = tf.arg_max(dist_ap, 1)
    ...
    # D_Negative = tf.reduce_max(D_Negative, 1)  # 18
    # dist_an = tf.reshape(D_Negative, [num_group, FLAGS.batch_size])
    V_neg = tf.reduce_max(dist_an, 1)  # 每一个原样本对应54个，挑一个最大的， 所以最后是18x18
...
def Loss(embedding, label, temperate, _lossType="none", loss_l2_reg=FLAGS.loss_l2_reg):
    """
    ...
    if _lossType == "Softmax":
        ...
        _Loss, _I_pos, _positive_emb, _I_neg, _negative_emb, _positive_label, _negative_label \
            = easy_positive_hard_negative_loss(
            embedding, label, temperate)
