...
def Pulling(Loss_type, embedding, Jm):
    ...
    #     return embedding_z_quta, label
        ...
        class0_emb, class1_emb = tf.split(embedding, 2, axis=0)


        embedding_z_quta = tf.concat([gen_class0, gen_class1], axis=0)
        return embedding_z_quta
    else:
        print("Your loss type is not suit for HDML")
        os._exit()
...
def get_embedding_aug(F, embeddings, labels, num_instance, n_inner_pts, l2_norm=True):
    ...
    for n_idx in range(n_pts):  # 合成点的个数
...
def cross_entropy(embedding, label, size=1024):  # 原本1024
    ...
    # Logits = tf.nn.relu(tf.matmul(embedding, W_fc) + b_fc)  # 调用的损失函数里已经有激活函数softmax了
    # 交叉熵就是labels * tf.log(softmax（Logits）)
    cross_entropy = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=label, logits=Logits))
    return cross_entropy, W_fc, b_fc, Logits
