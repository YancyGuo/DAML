...
def main(_):
    ...
    with tf.variable_scope('Classifier'):
        ...
        #     embedding, normal=FLAGS.normalize, is_Training=is_Training, name='BN1')
        # # FC layer 1
        # embedding_1 = nn_Ops.fc_block(
        #     embedding, in_d=1024, out_d=512,
        #     name='fc1', is_bn=False, is_relu=False, is_Training=is_Training)
        # # Batch Normalization layer 2
        # embedding_2 = nn_Ops.bn_block(
        #     embedding_1, normal=FLAGS.normalize, is_Training=is_Training, name='BN2')
        # # FC layer 2
        # embedding_z = nn_Ops.fc_block(
        #     embedding_2, in_d=512, out_d=FLAGS.embedding_size,
        #     name='fc2', is_bn=False, is_relu=False, is_Training=is_Training)

        # embedding_z = nn_Ops.fc_block(   # 未试
        #     embedding, in_d=1024, out_d=FLAGS.embedding_size,
        #     name='fc1', is_bn=False, is_relu=True, is_Training=is_Training)
        ...
        #
        # embedding = nn_Ops.bn_block(
        #     embedding_z, normal=FLAGS.normalize, is_Training=is_Training, name='BN2')
        #
        # # FC layer 1
        # embedding_z = nn_Ops.fc_block(
        #     embedding, in_d=512, out_d=FLAGS.embedding_size,
        #     name='fc2', is_bn=False, is_relu=False, is_Training=is_Training)
        # embedding_z = nn_Ops.fc_block(  # 加个激活层试试！！！！！！！！！！！！！！！！！！！！！
        #     embedding, in_d=1024, out_d=512,
        #     name='fc1', is_bn=False, is_relu=True, is_Training=is_Training)
        # embedding_z = nn_Ops.fc_block(  # 加个激活层试试！！！！！！！！！！！！！！！！！！！！！
        #     embedding_z, in_d=512, out_d=FLAGS.embedding_size,
        #     name='fc22', is_bn=False, is_relu=True, is_Training=is_Training)
        ...
        with tf.name_scope('Loss'):
            ...
            #embedding_l = embedding_z   + wdLoss
            J_m = Loss_ops.Loss(embedding_z, label,0.1, FLAGS.LossType)
    ...
    if FLAGS.Apply_HDML:
        ...
        embedding_z_quta = HDML.Pulling(FLAGS.LossType, embedding_z, J_m)  # 原来javg
        ...
        with tf.variable_scope('Classifier'):
            ...
            #     embedding_z_quta, in_d=1024, out_d=512,
            #     name='fc1', is_bn=False, is_relu=False, reuse=True, is_Training=is_Training)

            # # Batch Normalization layer 2
            # embedding_z_quta = nn_Ops.bn_block(
            #     embedding_z_quta, normal=FLAGS.normalize, is_Training=is_Training, name='BN2', reuse=True)
            # # FC layer 2
            # embedding_z_quta = nn_Ops.fc_block(
            #     embedding_z_quta, in_d=512, out_d=FLAGS.embedding_size,
            #     name='fc2', is_bn=False, is_relu=False, reuse=True, is_Training=is_Training)

        with tf.name_scope('Loss'):
            # J_syn 用的是生成样本，J_m用的是原始样本
            # 这里的loss函数和前面J_m的是一样的
            J_syn = (1. - tf.exp(-FLAGS.beta / Jgen)) * Loss_ops.Loss(embedding_z_quta, label, 0, _lossType=FLAGS.LossType)
            J_m2 = (tf.exp(-FLAGS.beta/Jgen))*J_m
        ...
        with tqdm(total=FLAGS.max_steps) as pbar:
            for batch in copy.copy(epoch_iterator):
                ...
                if not FLAGS.Apply_HDML:
                    ...
                    c_train, g_train, s_train, wd_Loss_var, J_metric_var, J_m_var, \
                    J_syn_var, J_recon_var, J_soft_var, J_gen_var, cross_en_var, _label_raw = sess.run(
                        [c_train_step, g_train_step, s_train_step, wdLoss,
                         J_metric, J_m, J_syn, J_recon, J_soft, J_gen, cross_entropy, label_raw],
                        feed_dict={x_raw: x_batch_data,
                                   label_raw: Label_raw,
                                   is_Training: True, lr: _lr, Javg: J_m_loss.read(), Jgen: J_gen_loss.read()})
                    ...
                    # r_step.append(step)
                    # r_mask.append(np.array(neg_mask_))
                    # r_neg.append(np.array(neg_))
                    # r_neg2.append(np.array(neg2_))
                    # np.savetxt
                    # io.savemat('tensor_save.mat', {'step': r_step, 'mask': r_mask, 'neg': r_neg, 'r_neg2': r_neg2})
                    ...
                    #     # cross_entropy_loss.update(cross_en_var)
                    #
                    #     train, s_train, wd_Loss_var, J_m_var, cross_en_var, _label_raw = sess.run(
                    #         [train_step, s_train_step, wdLoss, J_m, cross_entropy, label_raw],
                    #         feed_dict={x_raw: x_batch_data,
                    #                    label_raw: Label_raw,
                    #                    is_Training: True, lr: _lr})
                    #
                    ...
                    # else:
                    #     c_train, g_train, s_train, wd_Loss_var, J_metric_var, J_m_var, \
                    #     J_syn_var, J_recon_var, J_soft_var, J_gen_var, cross_en_var, _label_raw, neg2_, neg_, neg_mask_ = sess.run(
                    #         [c_train_step, g_train_step, s_train_step, wdLoss,
                    #          J_metric, J_m, J_syn, J_recon, J_soft, J_gen, cross_entropy, label_raw, neg2, neg,
                    #          neg_mask],
                    #         feed_dict={x_raw: x_batch_data,
                    #                    label_raw: Label_raw,
                    #                    is_Training: True, lr: _lr, Javg: J_m_loss.read(), Jgen: J_gen_loss.read()})
                    ...
                    #     wd_Loss.update(var=wd_Loss_var)
                    # if step <= 3000:
                    #     c_train, wd_Loss_var, J_metric_var,  _label_raw = sess.run(
                    #         [c_train_step, wdLoss,
                    #          J_metric, label_raw],
                    #         feed_dict={x_raw: x_batch_data,
                    #                    label_raw: Label_raw,
                    ...
                    # elif step <= 4000:
                    #     print('stage2')
                    #     g_train, s_train, J_m_var, \
                    #     J_syn_var, J_recon_var, J_soft_var, J_gen_var, cross_en_var, _label_raw = sess.run(
                    #         [g_train_step, s_train_step,
                    #          J_m, J_syn, J_recon, J_soft, J_gen, cross_entropy, label_raw],
                    #         feed_dict={x_raw: x_batch_data,
                    #                    label_raw: Label_raw,
                    #                    is_Training: True, lr: _lr, Javg: J_m_loss.read(), Jgen: J_gen_loss.read()})
                    #     J_m_loss.update(var=J_m_var)
