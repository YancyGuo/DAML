...
def Pulling(Loss_type, embedding, label):
    ...
    #     return embedding_z_quta, label
        ...
        # 插值2个
        inner_pts_1 = (reversed_class0_emb * 9 + reversed_class1_emb * 1) / 10
        inner_pts_2 = (reversed_class0_emb * 1 + reversed_class1_emb * 9) / 10
        inner_pts_11 = (class0_emb * 9 + class1_emb * 1) / 10
        inner_pts_22 = (class0_emb * 1 + class1_emb * 9) / 10

        # L2正则化
        inner_pts_1 = tf.nn.l2_normalize(inner_pts_1, axis=1)
        inner_pts_2 = tf.nn.l2_normalize(inner_pts_2, axis=1)
        inner_pts_11 = tf.nn.l2_normalize(inner_pts_11, axis=1)
        inner_pts_22 = tf.nn.l2_normalize(inner_pts_22, axis=1)
        # inner_pts_3 = tf.nn.l2_normalize(inner_pts_3, axis=1)

        # # 拼接
        concat_emb = tf.concat([embedding, inner_pts_1, inner_pts_2, inner_pts_11, inner_pts_22], axis=0)
