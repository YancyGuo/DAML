...
from lib import GoogleNet_Model, Loss_ops, nn_Ops, Embedding_Visualization, HDML111, evaluation
import copy
...
def main(_):
    ...
    if FLAGS.Apply_HDML:
        ...
        embedding_z_quta = HDML111.Pulling(FLAGS.LossType, embedding_z_recombine, J_m)  # 原来javg
        ...
        with tf.variable_scope('Generator'):
            ...
            embedding_original_sample, _, _ = tf.split(embedding_yp, 3, axis=0)
            embedding_yq_labels = tf.concat([label, positive_label, negative_label], axis=0)

            embedding_yp_img = nn_Ops.fc_block(
                embedding_original_sample, in_d=1024, out_d=FLAGS.default_image_size * FLAGS.default_image_size * 3,
                name='generator_img', is_bn=False, is_relu=False, is_Training=is_Training
            )

            embedding_yp_img = tf.nn.sigmoid(embedding_yp_img)
            embedding_yp_img = tf.reshape(embedding_yp_img, shape=[-1, FLAGS.default_image_size, FLAGS.default_image_size, 3])
        ...
        with tf.name_scope('Loss'):
            ...
            syn_sample_loss_loss = Loss_ops.Loss(embedding_z_quta, label, temperate=0.1, _lossType='epnh_synstage_loss',) # 这个label传进去是没有用到的
            ...
            J_recon = 0.000001* tf.reduce_sum(tf.squared_difference(unreshape, x_flat))
            ...
            # J_recon = (1 - FLAGS._lambda) * tf.reduce_sum(tf.square(embedding_yp - embedding_y_origin))
            # J_recon = tf.reduce_mean(tf.reduce_sum(tf.math.log(embedding_yp * embedding_y_origin)))
            ...
            cross_entropy, W_fc, b_fc = HDML111.cross_entropy(embedding=embedding_y_origin, label=embedding_y_origin_label)
        ...
        with tqdm(total=FLAGS.max_steps) as pbar:
            for batch in copy.copy(epoch_iterator):
                ...
                if not FLAGS.Apply_HDML:
                    ...
                    J_syn_var, J_recon_var, J_soft_var, J_gen_var, cross_en_var, _label_raw,\
                    ...
                    pbar.set_description("J_metric_var: %f, Jm: %f, J_syn_var: %f, "
                                         "J_recon_var: %f, J_soft_var: %f, J_gen_var: %f, CrossEntropy: %f, syn_factor: %f" %
                                         (J_metric_var, J_m_var, J_syn_var, J_recon_var, J_soft_var, J_gen_var, cross_en_var,  _syn_factor))
                ...
                if step % bp_epoch == 0:
                    ...
                    nmi_te, f1_te, recalls_te, recall, precision, f1_score, k_max_f1score = evaluation.Evaluation(
                        embedding_yp_img, stream_test, stream_train_eval, image_mean, sess, x_raw, label_raw, is_Training,
                        embedding_z, 2,
                        neighbours)
