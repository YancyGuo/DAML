...
这个文件包含一些在cnn中广泛使用的基本块
"""
from augMethod import MyAugMethod
...
def distance(emb1, emb2):
    ...
    :return: The distance
    """
    return tf.reduce_sum(tf.square(emb1-emb2), axis=1, keep_dims=True)
...
def training(loss, lr, var_scope='None', global_step=None, g_truncate=False, g_limit=2.):

# def training(loss, lr, var_scope='None', global_step=0, g_truncate=False, g_limit=2.):
    """
    ...
    with tf.name_scope('Training'):
        ...
        with tf.control_dependencies(extra_update_ops):
            if g_truncate:  # 默认false
                ...
                if var_scope != 'None':
                    # 好在主干网络也在 'Classifier'里面，如果是generator,则只训练这个scope的
                    var_list = tl.layers.get_variables_with_name(var_scope, True, True)   # 获取这个scope的变量
                    if var_scope == 'Classifier':
                        # 如果是Classifier，则加上的resnet_model？
                        var_list += tl.layers.get_variables_with_name('resnet_model', True, True)
                    train_step = tf.train.AdamOptimizer(lr).minimize(loss, global_step=global_step, var_list=var_list)
                else:
...
def batch_data(batch):
    ...
    if FLAGS.Apply_Augmentation:
        augmentator = MyAugMethod()  # NHWC
        data_list = [i for i in x_batch_data]
        x_batch_data = augmentator.aug_data(data_list)
