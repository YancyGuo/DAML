...
streams = data_provider.get_streams(FLAGS.batch_size, FLAGS.dataSet, 'n_pairs_mc', crop_size=FLAGS.default_image_size)
streams_unaug, stream_train, stream_train_eval, stream_test = streams
...
def main(_):
    ...
    if FLAGS.Apply_HDML:
        ...
        with tf.name_scope('Loss'):
            # loss_l2_reg_tensor = tf.constant(FLAGS.loss_l2_reg, dtype=tf.float32)
            r_loss, reg_anchor, reg_positive, l2loss, pos_tile, anc, pos, label2, label_anc, label_pos, label_anc, similarity_matrix, labels_remapped, x_loss = Loss_ops.new_npair_loss(
                labels=label,
                embedding_anchor=embedding_zq_anc,
                embedding_positive=embedding_zq_negtile,
                equal_shape=False, reg_lambda=FLAGS.loss_l2_reg)
            J_syn = (1. - tf.exp(-FLAGS.beta / Jgen)) * r_loss
