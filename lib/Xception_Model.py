...
def xception(inputs,
    ...
    with tf.variable_scope('Xception') as sc:
        ...
        with slim.arg_scope([slim.separable_conv2d], depth_multiplier=1), \
            ...
            net = slim.separable_conv2d(net, 1024, [3, 3], scope='block14_dws_conv2')
            ...
            # net = slim.avg_pool2d(net, [10, 10], scope='block15_avg_pool')  # 原网络
            # Replace FC layer with conv layer instead
            net = slim.conv2d(net, 1024, [1, 1], scope='block15_conv1') # 改
            net = tf.reshape(net, [-1, 1024])  # 增
