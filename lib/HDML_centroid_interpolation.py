...
def Pulling(Loss_type, embedding, label, Lambda_0, T, t):
    """
    ...
    #     return embedding_z_quta, label
        ...
        center_0 = tf.reduce_mean(class0_emb, axis=0)
        center_1 = tf.reduce_mean(class1_emb, axis=0)
        direct = (center_0 + center_1) / 2
        Lambda = Lambda_0/2*tf.math.exp((tf.math.exp(1.0)-1)/T*t + 1)
        Lambda = tf.clip_by_value(Lambda, 0, 6)
        # Lambda = Lambda_0 * tf.math.exp(t/T + 1)
        # Lambda = 4 * (tf.math.exp(t / T) - 1)
        # Lambda = tf.Print(Lambda, [Lambda], "Lambda")
        new_embedding = embedding + Lambda * direct
        # new_embedding = tf.nn.l2_normalize(new_embedding, axis=1)

        # center_0 = tf.reduce_mean(class0_emb, axis=0)
        # center_1 = tf.reduce_mean(class1_emb, axis=0)
        # direct_0 = (center_0 - center_1) / 2
        # direct_1 = (center_1 - center_0) / 2
        # # Lambda = Lambda_0 / 2 * tf.math.log((tf.math.exp(1.0) - 1) / T * t + 1)  # (0, 0.5)递增
        # # # Lambda = Lambda_0 / 2 * tf.math.log((tf.math.exp(1.0)) / T * t + 1)  # (0, 0.5)递增
        # # # Lambda = tf.Print(Lambda, [Lambda], "Lambda")
        # Lambda = 4 * (tf.math.exp(t/T)-1)
        # new_embedding_0 = class0_emb + Lambda * direct_0
        # new_embedding_1 = class1_emb + Lambda * direct_1

        # concat_embedding = tf.concat([embedding, new_embedding_0, new_embedding_1], axis=0)
        concat_embedding = tf.concat([embedding, new_embedding], axis=0)
        concat_label = tf.concat([label, label], axis=0)

        return concat_embedding, concat_label
    else:
        print("Your loss type is not suit for HDML")
        os._exit()
